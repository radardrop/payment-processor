...
describe("payment-processor", () => {
  ...
  before(async () => {
    ...
    [paymentProcessorPubkey] = anchor.web3.PublicKey.findProgramAddressSync(
  ...
  it("should initialize the payment processor", async () => {
    ...
    await program.methods
      .initialize(receiverKeypair.publicKey, rdaiMintPubkey)
      .accounts({
        // @ts-ignore
    ...
    expect(state.receiver.toBase58()).to.equal(
    ...
    console.log(
      "PaymentProcessor initialized with RDAI mint:",
      rdaiMintPubkey.toBase58()
    );
  });
  ...
  it("should pay correctly", async () => {
    ...
    expect(Number(payerAfter.amount)).to.equal(
      Number(payerBefore.amount) - Number(amount)
    );
    expect(Number(receiverAfter.amount)).to.equal(
      Number(receiverBefore.amount) + Number(amount)
    );
  ...
  it("should not pay correctly (attempt to pay with another mint)", async () => {
    ...
    try {
      await program.methods
        .pay(new anchor.BN(50_000), new anchor.BN(123))
        .accounts({
          payer: payerKeypair.publicKey,
      ...
      expect(e.error.errorMessage).to.include(
        "The mint of the provided token account does not match the PaymentProcessor"
      );
    }
  });
  ...
  it("should change the receiver", async () => {
    ...
    await program.methods
      .setReceiver(newReceiver.publicKey)
      .accounts({
        paymentProcessor: paymentProcessorPubkey,
        // @ts-ignore
    ...
    const state = await fetchPaymentProcessor(paymentProcessorPubkey);
    expect(state.receiver.toBase58()).to.equal(
      newReceiver.publicKey.toBase58()
    );
    ...
    await program.methods
      .setReceiver(receiverKeypair.publicKey)
      .accounts({
        paymentProcessor: paymentProcessorPubkey,
        // @ts-ignore
        owner: ownerKeypair.publicKey,
    ...
    expect(state2.receiver.toBase58()).to.equal(
      receiverKeypair.publicKey.toBase58()
    );

    console.log(
      "Receiver changed back to:",
      receiverKeypair.publicKey.toBase58()
    );
  });
  ...
  it("should emit an event when the payment is done", async () => {
    ...
    if (events.length) {
      ...
      expect(events[0].payer.toBase58()).to.equal(
        payerKeypair.publicKey.toBase58()
      );
